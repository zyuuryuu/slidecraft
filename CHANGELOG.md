...
- **マスター取り込みの透明化（進捗＋結果）** — .pptx を取り込む3経路（忠実 Import／決定論 Re-make／AI Re-make）で、処理中は進捗バー（AI は best-of-N の候補 {n}/{N} を表示）、処理後は結果要約バー（モード・健全性・レイアウト数・AI/フォールバック）を表示。「詳細」を開くと **AI 写像表（元 → 標準レイアウト＋理由）**・抽出テーマ（フォント/配色/ロゴ）・修復件数・健全性の指摘を確認できる（従来は無言適用 or 2.8 秒トーストのみ）。MasterPicker の ⓘ で再表示。ADR-0026 §9.1 で配線した `mappings`（reason 付き）を可視化。
