...
- **生原稿（`manuscript-*.md`）**：起動後 **📄 Import** → モーダルで本文を貼り付け（または
  「📄 ファイルを開く」）→ **✨ 原稿を整形** で見出しごとのスライドに → 帯の **整形レビュー**
  を確認 → **✓ 確定 → 編集** で Edit へ。`Initialize → 整形 → 診断 → 修復` の通しテスト。
- **構造化Markdown（`deck-*.md`）**：**📄 Import** → 「📄 ファイルを開く」で読込 →（必要なら
  整形/レビュー）→ **✓ 確定 → 編集**。Edit での図/表/レイアウト編集をテスト。
