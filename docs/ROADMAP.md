...
- **Live MCP でAIが新規Deckを作ってもGUIにタブが出ない（協働マルチドキュメント未統合）** — 協働ホストは multi-doc（`DocRegistry`）だが、GUI は seed した1ドキュメントだけを projection で mirror（`collab-projection.tick` の target は `targetDocId` 固定・`activeDocId` 不参照）。AI が `new_project` で新ドキュメントを mint すると host registry には増える（協働パネルの「ドキュメント: N」が増加）が、**GUI にタブが増えず・切替もできず不可視**（`onDocs` は `docCount` を更新するだけで `useDocumentStore.newDoc` 未配線）。**要設計判断**＝AI 作成 Deck を (a) 自動タブ化＋切替 / (b) タブ化するが切替えない / (c) 通知して開かせる、のどれにするか。触点: `useCollab`（`onDocs`/`documentOpened`）・`useDocumentStore.newDoc`・`DocTabs`・`CollabProjection.setTargetDoc`。
