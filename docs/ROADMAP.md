...
土台（テンプレ堅牢性）・差別化アーキ（内蔵 AI）・**プロンプト磨き込み（旧テーマ1）は完了**。次は 機能 フェーズ。
...
> **旧テーマ1「プロンプト磨き込み」＝完了**（2026-07-03）。構造ヘッダー保全（[ADR-0012](adr/0012-ai-edit-structure-preservation.md)）
> ＋敵対検証ハードニング＋#12/#13/#3B/図生成二段構え/#3/#1。詳細は下記の完了ログ＋git。
...
> - **#1 `slideSystemPrompt`（手動コピー用）の catalog 動的化 → 完了**：レイアウト名を canonical 固定から
>   **入力マスターの実レイアウトへ**（`slideSystemPrompt(catalog?)`：レイアウト一覧＋role ベースの選択ルールを
>   catalog から生成、無い role はルール省略＝canonical を混入させない。catalog を App→LlmAssist へ配線）。alien 対応
>   （[[guardrail_any_template]]）、`tests/slide-system-prompt.test.ts` +4。
> - **テーマ1 A（プロンプト磨き込み残課題）＝クローズ**：#12/#13/#3B/#12-5(C)/図生成二段構え/#3/#1 完了。
>   次リードは テーマ2「HTML 出力」or テーマ3「テンプレ作成補助」。
