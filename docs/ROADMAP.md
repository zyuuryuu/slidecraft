...
| **スライドマスター Re-make（テーマ抽出→自前レイアウト決め打ち）** 💬 DISCUSS | 実マスターの**構造を忠実に踏襲するのではなく、テーマ（フォント/文字サイズ/背景/ロゴ・装飾/配色）だけ抽出**し、SlideCraft 自前の canonical レイアウト（well-controlled な idx 体系）で placeholder を埋める新しい**取り込みモード**。**第三者マスターの placeholder 構造ゆらぎ（idx 衝突）を構造的に回避** — [ADR-0023](adr/0023-third-party-master-idx-convention.md) の残存エッジ（型付きメタ＋本文 idx10/15/16）を根絶する上位案。**実現性=高**：抽出側（`loadTemplate`：`themeColors`/`masterTitleStyle`/`masterBodyStyle`/`extractBackground`/`masterDecorations`=ロゴ）と生成側（`template-writer`：`TemplateSpec{fonts,palette}`→フル OOXML）が既存。新規は **(1)** `master→TemplateSpec` 抽出器（テーマ色→9色 palette・フォント major/minor＋EA/CJK 明朝/ゴシック分類）**(2)** `TemplateSpec`/writer に**背景＋ロゴ/装飾の注入**を追加 **(3)** 取り込み UI に「忠実取り込み ⇄ テーマだけ Re-make」の選択。**判断ポイント**：忠実取り込み（会社独自レイアウトを保持）と Re-make（自前レイアウト＝品質保証）を**両立モード**にするか。関連 [[master_intake_workflow]] | M |
