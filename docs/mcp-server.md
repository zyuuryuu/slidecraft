...
| 調達 | `list_templates()` | **host専用**：登録済みテンプレ一覧 `{templates:[{id,name,builtin}]}`（GUI が `register_templates` で投入した master レジストリ）。id を `use_template` へ。未接続は `{ok:false, code:"template-registry-unavailable"}`→`create_template` で代替 |
| 調達 | `use_template(id, markdown?)` | **host専用**：登録テンプレ（`list_templates` の id）から**新規プロジェクトを mint**（bytes 不要・GUI の Draft と同じ整形）。未知 id は never-silent `{ok:false, code:"unknown-template"}`。既存 doc のテンプレ入替ではない |
| 契約 | `get_authoring_guide()` | **著作の入口**：このテンプレのレイアウト名に解決した Markdown 書式・`<!-- col/kpi/step -->`・表/コード・`<!-- note -->` スピーカーノート・本文 budget＋図/spec ガイドへのポインタ |
...
| 読む | `get_slide(index)` | 1スライドの**構造化 read**：resolvedLayout・hasFigure/figureKind・bulletCount・budget・overBudget・当該 issues・notes（`<!-- note -->` スピーカーノート本文、無ければ null）・sectionBreak（章扉宣言）・derived（`"toc"`=導出専用の目次スライド、markdown はマーカー 1 行）・markdown（1呼び出しで編集計画） |
...
0. テンプレを調達：既存 `.scft` を base64 化して `open_project`、または `.pptx` を `new_project`。bytes が
   無ければ `create_template({preset:"midnight"})` → 返った `templateBase64` を `new_project` に渡す。**host（GUI 協働）では
   `list_templates` で登録済みテンプレを見て `use_template(id, markdown?)` で選ぶ**のが最短（bytes を運ばない）。
