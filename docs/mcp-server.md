...
- 現時点は**入出力の fs のみ**（#299 段階1・3）。loopback binary 配信（②・fs 無し環境向け）は follow-up
  （ADR-0035）。
...
- scoped fs（`--root`）は入出力とも実装済み（ADR-0035 段階1＝出力・PR #304/#305、段階3＝入力・#299）。
  MCP `roots`（クライアント advertise・`--root` の代替経路）と loopback binary 配信（②・fs 無し環境向け）
  は follow-up。
