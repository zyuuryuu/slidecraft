...
- **`--root <dir>`（opt-in・ADR-0035 段階1＝出力側）**：指定すると `export_pptx`/`save_project` は
  base64 の代わりに **`<dir>` 配下へファイルを書き、パスを返す**（大きいデッキで base64 の +33% と
  AI コンテキスト圧迫を避ける）。既定（`--no-fs`）は非破壊で温存。詳細は下記「scoped fs 出力」。
...
既定は `--no-fs`（base64）。`--root <dir>` を渡すと scoped fs 出力（下記）が有効になる。登録後、エージェントから下記ツールが見える。
...
5. `export_pptx` の `dataBase64` を**自分で `.pptx` に書き出す**（または `save_project` で `.scft` を保存）。
   サーバが `--root <dir>` scope で起動していれば、代わりに `{path}` が返る（下記「scoped fs 出力」）。
...
## scoped fs 出力（`--root`・ADR-0035 段階1）
...
`export_pptx` / `save_project` は base64 の代わりに **`<dir>` 配下へファイルを書き、参照（`{path}`）を返す**。

- **opt-in・非破壊**：`--root` を渡さない既定は従来どおり `{dataBase64}`（byte-identical）。
- **スコープは 1 ディレクトリに封じる**：`--root` に渡したディレクトリの**実体パス**（symlink 解決済み）だけが
  書き込み対象。`../` traversal・絶対パス・サブディレクトリ・symlink 越えは**すべて拒否**（`{ok:false,
  code:"scope-violation"}`・never-silent）。`filename` は拡張子（`.pptx` / `.scft`）込みのベア名のみ許可。
- **`filename` は任意**：省略するとテンプレ名/タイトルとタイムスタンプから自動生成。scope 未設定なのに
  `filename` を渡すとエラー（`code:"scope-not-configured"`）＝黙って無視しない。
- **返り値**：`{path: "file://<root>/<filename>"}` — **絶対** file URI（`<root>` は `--root` の実体パス）。
  `file:///<filename>` のような bare 形式は RFC 8089／`new URL()` 解釈では「絶対パス `/<filename>`」になり
  fs ルート直下を誤って指すため使わない（#299 follow-up）。標準パーサ（`new URL(path).pathname` /
  Node の `fileURLToPath(path)`）でそのまま実ファイルに解決できる。`root` は呼び出し側が自分で渡した
  `--root` の値なので、絶対パスの echo はリークにならない。`path` フィールドは scope 起動時のみ現れ、
  未指定時は代わりに `dataBase64` が現れる（両方が同時に返ることはない）。
- 現時点は**出力側のみ**（#299 第1段）。`open_project`/`new_project`（入力側）と loopback binary 配信は
  follow-up（ADR-0035）。

```bash
slidecraft serve --root /path/to/scope   # scoped fs 出力を有効化
```
...
- scoped fs（`--root`）は**出力側のみ**（ADR-0035 段階1・#299）。`open_project`/`new_project`（入力側）は
  引き続き base64 専用。MCP `roots`（クライアント advertise・`--root` の代替経路）と loopback binary
  配信（fs 無し環境向け）は follow-up。
