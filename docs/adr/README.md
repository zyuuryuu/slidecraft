...
| [0007](0007-mcp-server-design.md) | MCP サーバ設計（決定論レバー・native-only export・--no-fs） | Accepted（stdio 単一管制は [0033](0033-mcp-single-control-plane.md) で部分 supersede・`--no-fs のみ`は [0035](0035-mcp-bulk-data-exchange.md) で部分 supersede・他は存続） |
...
| [0032](0032-authoring-notes-and-sections.md) | authoring 記法拡張＝`<!-- note -->` ノートマーカー＋`<!-- section -->` 章タグ（章扉は生成でなく著者スライド＝ADR-0002/0030 との衝突回避・目次は `<!-- toc -->` 導出専用で複製状態を持たない=R8・新記法なし md は byte-identical） | Accepted（「便利スライドの生成は行わない」方針は [0034](0034-convenience-slide-generation.md) で部分 supersede・live 導出/ノート/章タグは存続） |
...
| [0034](0034-convenience-slide-generation.md) | 便利スライドの生成＝live/static ハイブリッド（GUI 生成メニュー・各タイプで live＝drift-proof 導出／static＝編集可能な普通スライドを選択・目次は live 既定）。ADR-0032 D2 の「生成却下」をモード選択で解消・特殊記法は手書き必須でなくなる | Accepted（目次から実装・他タイプ後続） |
| [0035](0035-mcp-bulk-data-exchange.md) | MCP バルクデータ授受＝バイトを JSON-RPC に載せず参照渡し。data plane は **fs（`--root`・入出力とも scoped dir に封じる）→ base64**（②loopback binary は不採用＝forward AI が token を持てず届かない・fs/projection で充足）。ADR-0007 の「--no-fs のみ」を部分 supersede | Accepted（①出力 #304・③入力 #306 実装済み・②不採用） |
