...
| [0035](0035-mcp-bulk-data-exchange.md) | MCP バルクデータ授受＝データプレーン階段（バイトを JSON-RPC に載せず参照渡し・fs〔roots→--root〕→ loopback binary → base64 の順で never-silent フォールバック・入出力とも scoped dir に封じる）。ADR-0007 の「--no-fs のみ」を部分 supersede・base64 は隔離時の最終手段に降格 | Accepted（#299・出力側先行の段階実装） |
