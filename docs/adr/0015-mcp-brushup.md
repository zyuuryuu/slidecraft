...
- **Status**: Accepted（設計確定・実装は S1–S6 で順次・手前半 T3 先頭。本 ADR は着手＝決定時点で執筆）
- **Date**: 2026-07-04
...
0-B. **テンプレ discovery/provisioning**：`list_templates`/`use_template(id)`（**host モード**・GUI の master レジストリを
   `HostContext` 経由で橋渡し＝`list_documents` と同じ host 能力）＋ `create_template(spec)`（AI が `TemplateSpec` を
   著作→harness 検証→[template-writer](../../src/engine/template-writer.ts) が生成・ADR-0014「AI は spec 提案のみ」踏襲・
   両モード可）。提出は既存 `new_project(templateBase64)`。
...
**代償・限界**
- 本 ADR は決定時点で執筆（ADR README の「実装済み」不変条件の例外）。**実装は S1–S6 で順次**（手前半 S1–S2 先頭）。
  各スライス完了時に `docs/mcp-server.md`（tool 一覧・典型ループ）と本 ADR の Status/追記を同期する。
