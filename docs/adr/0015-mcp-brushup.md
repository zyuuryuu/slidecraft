...
- **Status**: Accepted（**S1–S6 実装完了 2026-07-04**・S2 増分2〔`list_templates`/`use_template`〕はバックログ）
- **Date**: 2026-07-04
...
**代償・限界**
- **S2 増分2（`list_templates`/`use_template`）はバックログ**：GUI の master レジストリ（React/Tauri 側）を `HostContext` に
  accessor 注入する host 機能で、GUI 側実装と対。stdio は `create_template`／bytes 持参で代替可。
