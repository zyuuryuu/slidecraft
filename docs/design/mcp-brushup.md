...
### G. テンプレ discovery / provisioning（T3・S2）

①「調達（選択/作成）」を埋める。提出（`new_project(templateBase64)`）は既存。

| 追加 | signature | 戻り | 実装・モード |
|---|---|---|---|
| `list_templates` | `()` | `{templates: [{id, name, health}]}` | **host モード**：GUI の master レジストリを橋渡し（`list_documents` と同じ host 能力）。stdio は非対応（AI が bytes 持参）。`HostContext` に template registry accessor を追加 |
| `use_template` | `(id)` | `{templateBase64}` or host で新規 doc mint | レジストリから bytes を解決。`new_project` に渡すか host で開く |
| `create_template` | `(spec)` | `{templateBase64, health}` | **両モード可**（純エンジン）。AI が `TemplateSpec` を著作 → harness 検証・正規化・コントラストガード（ADR-0014）→ [template-writer](../../src/engine/template-writer.ts) が PPTX 生成。host では併せてレジストリ登録も可。サーバはモデルを呼ばない（spec は AI 著作） |
| `get_template_spec_guide` | `()` | `templateSpecSystemPrompt()` の文字列＝TemplateSpec の書式ガイド | `template-spec-prompts.ts` 再利用・両モード可（L3・`create_template` とペア） |

> S2 の未決フォーク＋red-team 指摘（調達クラスタで詰める）：
> - **レジストリは engine/host から届かない**：master レジストリは React hook `useMasterRegistry`＋Tauri 専用
>   `src/ipc/master-store.ts` で、`HostContext` から参照できる engine オブジェクトが無い。→ list/use_template は
>   **GUI が registry accessor を HostContext に注入**する host 能力として設計する（engine 直参照ではない）。
> - **stdio では①が閉じない**：list/use は host 専用・create は full TemplateSpec 著作が要る＝bytes を持たない素の
>   stdio AI が始められない。→ **canonical を built-in default として stdio でも `use_template("default")` で開ける**
>   ようにするか（＝「headless で select」フォークと統合）。
> - create_template を **preset＋override** にして spec 著作負担を下げるか／人間可読で安定な `id` スキーム。

`create_template` は「作る」だけ＝返した bytes を `new_project` に渡して着手（`new_project` が bytes を取るのと対称・
composable）。ADR-0014「AI は spec 提案のみ・PPTX を書くのは決定論コード」を MCP に踏襲。
