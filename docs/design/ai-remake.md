...
1. この設計の**方針確定**（特に §4 の A/B/C＝推奨 C）。→ **確定（C）**
2. Phase-0 スパイクの実装（feasibility 計測）→ GO/NO-GO。→ **GO**
3. GO なら ADR 化（決定の記録）＋ Phase-1 MVP を test-first で。→ **[ADR-0026](../adr/0026-ai-remake.md) Accepted・Phase-1 出荷済**

## 9. Phase-2（説明可能性 + best-of-N）— 実装済

Phase-1 MVP（単発 callAI → 写像 → 決定論フォールバック）に2点を足した:

### 9.1 写像の根拠（説明可能性・§7 の任意項目を実装）

- プロンプトの各エントリに `reason`（`{ base, rename, reason }`）を要求。`parseRemakeMapping` が
  trim + 200字上限で捕捉、`aiRemakeSpec` が `mappings`（`MappedLayout[]`）として返す。
- UI はトーストで要約（`AI が N レイアウトを再構成しました` / 未使用時は決定論フォールバック文言）。
  per-layout の「なぜ」パネルは今後の増分（`mappings` は配線済で載せるだけ）。

### 9.2 best-of-N（ローカル小モデルの run 間ばらつき対策）

- `pickBestRawMapping(raws[])` — N 回サンプリングし、**最も多くの source を写像**（同点は
  **ハルシネーション drop 最少**）した raw を採用。純関数（clock/rng 非依存＝resume 安全）。
- `applyTemplateBytesAsRemakeAI(..., { n })` が `n` 回 callAI（逐次）→ `pickBestRawMapping`。
  既定 `REMAKE_BEST_OF_N = 2`（App）。AI 未接続・cloud 経路は `n=1` に潰れる。

### 9.3 モデルばらつき実測（CX_sample = 22 source layouts, Ollama, K=3 runs/model）

| model (size)            | valid | mapped (min..max, mean) | dropped (mean,max) | 所見 |
|-------------------------|-------|-------------------------|--------------------|------|
| **phi4** (14B)          | 3/3   | 22..22, **22.0**        | 0.0, 0             | 完璧・0分散 |
| **granite4.1:8b** (8B)  | 3/3   | 21..21, 21.0            | 0.3, 1             | 準完璧・安定（既定候補） |
| llama3.1:8b (q4_K_M)    | 3/3   | 20..22, 21.0            | 1.0, 2             | 良好 |
| phi3.5 (3.8B)           | 3/3   | 15..20, 18.3            | 1.0, 3             | 弱・ばらつき大（best-of-N が効く） |
| mistral:7b (7B)         | 3/3   | 9..23, 14.0             | 7.0, 11            | 高ばらつき＋高ハルシ・不安定 |

*(granite4.1:30b は本機で 1 run が計測窓を超過し除外。5 モデルで結論は十分。)*

**結論**:
1. **valid-JSON は全 5 モデルで 3/3** — parse floor は堅く、全滅→決定論フォールバックは稀。
   ハルシネーション base は `parseRemakeMapping` が drop するので **health は常に ok**。
2. **強モデル（phi4 / granite8b / llama）はばらつきが小さく**、best-of-N の限界効用は小
   （既に天井付近）。**弱・不安定モデル（phi3.5 / mistral）で best-of-N が効く**（best ≫ mean）。
3. → **既定 `n=2` が妥当**：強モデルには ~30s の保険、弱モデルには floor 押し上げ。三重化は
   不安定モデルの mistral でも drop 過多で報われないため見送り。
4. **既定モデル推奨**: **phi4**（完璧）または **granite4.1:8b**（準完璧・現行 spike 既定）。
   小さすぎるモデル（phi3.5 3.8B）は弱く、best-of-N で緩和しつつも非推奨。

### 9.4 残（Phase-3 候補）

- **案 B の部分導入** — canonical に無い source 固有構造を AI が幾何ごと合成（§4 B）。
- **per-layout「なぜ」パネル** — `mappings`（reason 付き）の可視化 UI。
- weaker-model の更なる緩和（プロンプト few-shot 化 等）。
