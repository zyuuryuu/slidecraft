...
このページでは、テンプレートの 3 つの入り口を扱います。

1. **既存 `.pptx` の取り込み** — 会社標準のテンプレートをそのまま使う
2. **壊れたテンプレートの修復取り込み** — 枠の役割が壊れた `.pptx` を「整形して取り込む」
3. **テンプレートの新規作成** — 配色・フォントを選んでゼロから作る（AI 提案・ライブプレビュー付き）
