...
| macOS | Homebrew cask（推奨）/ `.dmg` | 下記コマンド。公式配布は **Apple Silicon（arm64）のみ**（`_aarch64.dmg`）。Intel Mac はソースからビルド |
...
::: tip Intel Mac をお使いの場合
公式インストーラ（cask／`.dmg`）は **Apple Silicon（arm64）のみ**です（CI ランナー事情により Intel 版は廃止）。
Intel Mac では[ソースからの開発起動](#ソースからの開発起動)の手順でビルドしてください。
:::
...
Homebrew でインストールすると、上流 AI（Claude Code / Cursor / Claude Desktop）から SlideCraft を駆動する MCP サーバも**同梱・PATH 登録**されます（v0.2.0 以降）。ソースの clone もシステム Node も不要で、`claude mcp add slidecraft -- slidecraft-mcp` で登録できます。詳細は [MCP ガイド](/guide/mcp) を参照してください。
:::
