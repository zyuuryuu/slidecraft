...
describe("Dirty_Grouped — スケール付きグループ内のタイトル生テキスト", () => {
  ...
  // #142 fix: extractStaticTexts はグループ変換（chOff/chExt スケール）を walkShapes と同じ
  // composeXf で合成する。fixture のグループは off={x:2,y:1.25,w:8,h:2} / chOff={0,0} chExt={4,1}
  // → sx=sy=2, tx=2, ty=1.25。子座標 x=0.5,y=0.25,w=3,h=0.5 はスライド座標 x=3,y=1.75,w=6,h=1 になる
  // （フォントサイズ 40pt はグループスケールで変わらない — PowerPoint 実挙動）。
  it("グループ内 staticText の幾何はグループ変換を合成してスライド座標になる", async () => {
    const tpl = await load("Dirty_Grouped_TemplateOnly.pptx");
    const cover = tpl.layouts.find((l) => l.name.includes("表紙"))!;
    const title = cover.staticTexts.find((s) => s.text.includes("年次報告 2026"))!;
    expect(title.style.x).toBeCloseTo(3, 1);
    expect(title.style.y).toBeCloseTo(1.75, 1);
    expect(title.style.w).toBeCloseTo(6, 1);
    expect(title.style.h).toBeCloseTo(1, 1);
    expect(title.style.fontSize).toBeCloseTo(40, 0); // スケールで変わらない
  });

  // 幾何が合成された結果、表紙の生テキスト見出しが「上部・広幅・大フォント」の title-as-static-text
  // 病理判定条件（master-pathology.ts:175）を満たすようになる ＝ census が clean と誤判定しなくなる。
  it("幾何合成の結果、census が表紙を title=生text で検出する（clean でなくなる）", async () => {
    const tpl = await load("Dirty_Grouped_TemplateOnly.pptx");
    const r = detectPathologies(tpl, "Dirty_Grouped");
    expect(r.counts["title-as-static-text"] ?? 0).toBe(1);
  });
});
