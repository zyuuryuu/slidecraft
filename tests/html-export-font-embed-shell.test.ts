 ...
 * #115). Locks: embeddedFonts renders one @font-face per face with a data:font/ttf base64 src (raw
 * sfnt, not WOFF2 — see font-subsetter.ts for why); absent/empty embeddedFonts is byte-identical to
 * a pre-embedding export (do-no-harm invariant).
 */
...
describe("HTML export: @font-face embedding (html-shell)", () => {
  it("injects an @font-face with a data:font/ttf base64 src for each embedded face", () => {
    const doc = assembleHtmlDeck(["<div>x</div>"], {
      ...STAGE,
      embeddedFonts: [{ family: "Noto Sans CJK JP", weight: 400, ttfBase64: "AAAA" }],
    });
    expect(doc).toContain("@font-face");
    expect(doc).toMatch(/font-family:"Noto Sans CJK JP"/);
    expect(doc).toMatch(/font-weight:400/);
    expect(doc).toContain('src:url(data:font/ttf;base64,AAAA) format("truetype")');
  });
  ...
  it("emits a separate @font-face rule per weight for the same family (regular + bold)", () => {
    const doc = assembleHtmlDeck(["<div>x</div>"], {
      ...STAGE,
      embeddedFonts: [
        { family: "Noto Sans CJK JP", weight: 400, ttfBase64: "REG" },
        { family: "Noto Sans CJK JP", weight: 700, ttfBase64: "BOLD" },
      ],
    });
    expect((doc.match(/@font-face/g) ?? []).length).toBe(2);
    expect(doc).toMatch(/font-weight:400;font-display:swap;src:url\(data:font\/ttf;base64,REG\)/);
    expect(doc).toMatch(/font-weight:700;font-display:swap;src:url\(data:font\/ttf;base64,BOLD\)/);
  });
