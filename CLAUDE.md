...
読み手（疎結合セッション・後から入る人）が文脈を復元できることを最優先する。対象は
**chat・Issue・PR・commit・docs のすべて**。

- **ジャーゴン（専門用語・プロジェクト固有語）は控えめに**。同じことを平易に書けるなら平易に書く。
- **使うときは、初出でまず 1 行の定義を添えてから使う**。以降は用語だけでよい。
  例：「golden（座標の期待値を固定したスナップショットテスト）に較正する」→ 次からは「golden」。
- **会話固有の略語（`②a` 等）は本文で 1 語説明する**（Issue 自己完結基準と同旨）。定義なしに
  持ち込まない。
